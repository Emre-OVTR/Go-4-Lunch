package emre.turhal.go4lunch.ui.restaurants_list;

import static emre.turhal.go4lunch.utils.Constants.BASE_URL_PLACE_PHOTO;
import static emre.turhal.go4lunch.utils.Constants.CLOSED;
import static emre.turhal.go4lunch.utils.Constants.CLOSING_SOON;
import static emre.turhal.go4lunch.utils.Constants.MAX_HEIGHT;
import static emre.turhal.go4lunch.utils.Constants.MAX_WIDTH;
import static emre.turhal.go4lunch.utils.Constants.OPEN;
import static emre.turhal.go4lunch.utils.Constants.OPENING_HOURS_NOT_KNOW;
import static emre.turhal.go4lunch.utils.FormatTime.formatTime;
...
import emre.turhal.go4lunch.databinding.FragmentRestaurantItemBinding;
import emre.turhal.go4lunch.models.googleplaces_gson.ResultDetails;
import emre.turhal.go4lunch.utils.DisplayRating;
        ...
        if (!(resultDetails.getPhotos() == null)) {
            if (!(resultDetails.getPhotos().isEmpty())) {
                glide.load(BASE_URL_PLACE_PHOTO + "?maxwidth=" + MAX_WIDTH + "&maxheight=" + MAX_HEIGHT + "&photoreference=" + resultDetails.getPhotos().get(0)
                                .getPhotoReference() + "&key=" + "AIzaSyA41i5Z_94xwl6x4KxcwDPUZxR-2CJ-2g8")
