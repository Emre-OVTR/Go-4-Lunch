...
import static emre.turhal.go4lunch.utils.Constants.MAX_HEIGHT_LARGE;
import static emre.turhal.go4lunch.utils.Constants.MAX_WIDTH_LARGE;
import static emre.turhal.go4lunch.utils.DisplayRating.displayRating;
import static emre.turhal.go4lunch.utils.GetTodayDate.getTodayDate;
import static emre.turhal.go4lunch.utils.ShowToastSnack.showToast;
...
import emre.turhal.go4lunch.databinding.ActivityDetailsBinding;
import emre.turhal.go4lunch.firebase.manager.BookingManager;
import emre.turhal.go4lunch.firebase.manager.UserManager;
import emre.turhal.go4lunch.models.Workmate;
import emre.turhal.go4lunch.models.googleplaces_gson.ResultDetails;
import emre.turhal.go4lunch.retrofit.googleplace.GooglePlaceDetailsCalls;
    ...
    private void updateUI(ResultDetails resultDetails) {
        ...
        if (resultDetails.getPhotos() != null) {
            Glide.with(this).load(BASE_URL_PLACE_PHOTO + "?maxwidth=" + MAX_WIDTH_LARGE + "&maxheight=" + MAX_HEIGHT_LARGE + "&photoreference=" + resultDetails.getPhotos().get(0)
                    .getPhotoReference() + "&key=" + "AIzaSyA41i5Z_94xwl6x4KxcwDPUZxR-2CJ-2g8").into(mBinding.imageView);
